...
def plot_model_comparison(results: list, save_path: str = None):
    ...
    def _bar_ax(ax, values, errs, title, ylabel, fmt=".4f"):
        ...
        for i, (bar, val) in enumerate(zip(bars, values)):
            ...
            ax.text(bar.get_x() + bar.get_width()/2,
                    ...
                    f"{val:{fmt}}", ha="center", color=color,
    ...
    plt.suptitle("🏆 Comparaison des Modèles NOTAM — Baseline",
...
def plot_confusion_matrices(results: list, classes: list, save_path: str = None):
    ...
    for ax, result in zip(axes, results):
        ...
        ax.set_title(
            result.model_name.replace(" (Calibrated)", ""),
    ...
    plt.suptitle("🔍 Matrices de Confusion Normalisées",
                 color="white", fontsize=14, y=1.02)
    plt.tight_layout(pad=3)
    if save_path:
        plt.savefig(save_path, dpi=150, bbox_inches="tight", facecolor="#0d1117")
...
def plot_per_class_f1(results: list, classes: list, save_path: str = None):
    ...
    ax.set_title("📊 F1-Score par Classe et par Modèle",
                 color="white", fontsize=13, pad=15)
    ax.legend(fontsize=9, framealpha=0.2, labelcolor="white",
              facecolor="#1c1c2c")
    ax.spines[:].set_visible(False)
    ax.tick_params(colors="#555")
    ax.axhline(0.85, color="gold", linestyle="--", linewidth=1,
               alpha=0.6, label="Cible ≥ 0.85")
...
def plot_learning_curve(model, X_train, y_train,
    ...
    ax.plot(train_sizes, train_mean, "o-", color="#3b82f6",
            linewidth=2.5, label="Train F1", markersize=7)
    ax.fill_between(train_sizes, train_mean - train_std,
                    train_mean + train_std, alpha=0.15, color="#3b82f6")

    ax.plot(train_sizes, val_mean, "s-", color="#10b981",
            linewidth=2.5, label="Validation F1", markersize=7)
    ax.fill_between(train_sizes, val_mean - val_std,
                    val_mean + val_std, alpha=0.15, color="#10b981")

    ax.axhline(0.85, color="gold", linestyle="--", linewidth=1.2,
               alpha=0.7, label="Cible ≥ 0.85")
    ...
    diagnosis = "✅ Good fit" if gap < 0.05 else ("⚠️  High variance" if train_mean[-1] > val_mean[-1] + 0.05 else "⚠️  High bias")
    ax.text(0.02, 0.05, f"Gap train/val = {gap:.3f} → {diagnosis}",
